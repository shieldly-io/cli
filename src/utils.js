...
/**
 * Markdown for a shareable score badge, e.g. for a README. Score may be null (unscanned).
 * Renders our own SVG directly. For shields.io's badge styling/options instead, embed via
 * their endpoint schema: https://img.shields.io/endpoint?url=<base>/api/badge/endpoint?score=N
 */
export function badgeMarkdown(score) {
  const base = getWebBase();
  const query = score === null || score === undefined ? '' : `?score=${encodeURIComponent(score)}`;
  const badgeUrl = `${base}/api/badge${query}`;
  return `[![Shieldly Security Score](${badgeUrl})](${base})`;
}
