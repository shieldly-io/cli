...
export async function analyzeCF(args) {
  ...
  if (stat.isDirectory()) {
    ...
    if (args.includes('--badge') && allResults.length > 0) {
      // Worst score across the batch — the most conservative signal for a badge.
      const scores = allResults.map(({ data }) => data.score).filter((s) => typeof s === 'number');
      const worst = scores.length > 0 ? Math.min(...scores) : null;
      const md = badgeMarkdown(worst);
